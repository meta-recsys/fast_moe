...
import triton
from fast_moe.kernels.moe import index_select_jagged_bmm, index_select_jagged_bmm_swiglu

from fast_moe.kernels.triton.triton_moe import _index_select_jagged_bmm_swiglu
from fast_moe.kernels.utils import KernelType
from torch.nn import functional as F
from torch.profiler import profile

# buck2 run @mode/{opt,inplace} //fast_moe/kernels/benchmarks:index_select_jagged_bmm_swiglu_bench -- --fwd-only
# buck2 run @mode/{opt,inplace} //fast_moe/kernels/benchmarks:index_select_jagged_bmm_swiglu_bench -- --dump-cache-dir=/home/${USER}/fbsource/fbcode/hammer/ops/triton/cc/index_select_jagged_bmm/autotune_cache.pkl
...
def main(
    ...
    def bench_index_select_jagged_bmm_swiglu(
        num_tokens: int,
        e: int,
        k: int,
        m: int,
        ...
        if provider in ["triton", "pytorch"]:
            fn = lambda: index_select_jagged_bmm_swiglu(  # noqa E731
                max_seq_len=max_seq_len,
                offsets=offsets,
                index=index,
                jagged=jagged,
    ...
    bench_index_select_jagged_bmm_swiglu.run(print_data=True)

    if dump_cache_dir:
        with open(dump_cache_dir, "wb") as data:
            # @lint-ignore PYTHONPICKLEISBAD
            pickle.dump(_index_select_jagged_bmm_swiglu.cache, data)
