...
@triton.jit
def _fused_jagged_jagged_bmm_reduce_sum(
    seq_offsets,
    JaggedA,
    JaggedB,
    Out,
    ...
    if REDUCE_JAGGEDB:
        out_reduce_ptrs = ReduceOut + off_b * stride_orb + offs_n * stride_orn
        acc_reduce = tl.zeros((BLOCK_K, BLOCK_N), dtype=tl.float32)
    if seq_len == 0:
        out = accumulator.to(Out.dtype.element_ty)
        tl.store(out_ptrs, out.T, mask=(offs_n[:, None] < N) & (offs_m[None, :] < M))
        if REDUCE_JAGGEDB:
            if off_m == 0:
                tl.store(
                    out_reduce_ptrs,  # pyre-ignore [61]
                    tl.sum(acc_reduce, axis=0).to(  # pyre-ignore [61]
                        ReduceOut.dtype.element_ty
                    ),
                    mask=(offs_n < N),
                )
        return
    ...
    if REDUCE_JAGGEDB:
        if off_m == 0:
            r_out = tl.sum(acc_reduce, axis=0).to(  # pyre-ignore [61]
                ReduceOut.dtype.element_ty
            )
            tl.store(
                out_reduce_ptrs,  # pyre-ignore [61]
...
def triton_jagged_bmm_reduce_sum(
    ...
    d_bias = torch.empty((B, N), dtype=dtype, device=device) if reduce_sum else None

    use_fused_reduce_sum = True
    if use_fused_reduce_sum and reduce_sum:
        grid = lambda meta: (  # noqa E731
        ...
        _fused_jagged_jagged_bmm_reduce_sum[grid](
            seq_offsets=offsets,
            JaggedA=JaggedA,
            JaggedB=JaggedB,
            Out=d_weight,
            ...
            M=M,
            N=N,
            K=triton.next_power_of_2(JaggedA.shape[0]),  # for autotune
            ...
            REDUCE_JAGGEDB=d_bias is not None,
            ALLOW_TF32=torch.backends.cuda.matmul.allow_tf32,
        )
        return d_weight, d_bias
...
def triton_silu_jagged_bmm_combine(
    ...
    option = SiluJaggedBmmCombineOption(
        ...
        d_weight_optimization=d_weight_optimization,
        d_weight_split_k_kernel=d_weight_split_k_kernel,
        d_weight_split_k_kernel_tma=d_weight_split_k_kernel_tma,
    )
